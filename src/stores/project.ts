...
import type { ProjectData, MatchedPixel, MaterialItem, PixelData } from '@/types'
...
export const useProjectStore = defineStore('project', () => {
  ...
  // 色盘匹配
  const selectedBrand = ref('perler')
  ...
  function saveProjectsToStorage() {
    localStorage.setItem('perler-beads-projects', JSON.stringify(projects.value))
  }
  ...
  async function doPixelate() {
    ...
    const result = pixelate(originalImage.value, pixelWidth.value, pixelHeight.value)
  ...
  async function doColorMatch() {
    ...
    const palette = getPaletteColors(selectedBrand.value)
  ...
  async function saveCurrentProject() {
    ...
    const project: ProjectData = {
      ...
      selectedPalette: selectedBrand.value,
    ...
    const existingIdx = projects.value.findIndex(
      p => p.originalImage === originalDataUrl.value && p.pixelWidth === pixelWidth.value
    )
    if (existingIdx >= 0) {
      projects.value[existingIdx] = { ...projects.value[existingIdx], ...project }
    } else {
  ...
  function loadProject(project: ProjectData) {
    ...
    selectedBrand.value = project.selectedPalette
